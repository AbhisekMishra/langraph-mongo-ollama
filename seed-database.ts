...
function normalizeKeys(data: any): any {
  if (Array.isArray(data)) {
    ...
    for (const key in data) {
      const normalizedKey = key.replace(/е/g, "e"); // Replace Cyrillic 'е' with English 'e'
      normalizedObject[normalizedKey] = normalizeKeys(data[key]);
    }
...
async function createEmployeeSummary(employee: Employee): Promise<string> {
  return new Promise((resolve) => {
    ...
    const workLocation = `Works at ${employee.work_location.nearest_office}`;
